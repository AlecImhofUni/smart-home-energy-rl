...
def plot_example_day(day_data: dict, results_dir: Path) -> list[Path]:
    ...
    # Solar profile: actual production vs noisy forecast
    fig, ax = plt.subplots(figsize=(8, 4))

    solar_actual = day_data.get("solar_actual", day_data["solar"])
    solar_forecast = day_data.get("solar_forecast")

    ax.plot(hours, solar_actual, marker="o", label="Actual solar")

    if solar_forecast is not None:
        ax.plot(hours, solar_forecast, marker="x", linestyle="--", label="Forecast solar")
